...
export const RateModal: FunctionComponent<RateModalProps> = (props) => {
    const commentCharacterLimit = 250;
    ...
    const handleComments = (comment: string) => {
        setComments(comment.length > commentCharacterLimit ? comment.substring(0, commentCharacterLimit) : comment);
    }
    ...
    return (
        <Modal isOpen={props.isOpen} toggle={props.toggle} className={styles.modal}>
            <div className={styles.header}>
                Rate
                <svg className={styles.x} onClick={props.toggle} width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <ModalBody className={styles.body}>
                ...
                <div className={styles.textareaHolder}>
                    <textarea className={styles.textarea} id="comment" onChange={(e) => handleComments(e.target.value)} value={comments}></textarea>
                    <div className={styles.letterCount}>{comments.length}/{commentCharacterLimit}</div>
                </div>
                <div className={styles.inputRow}>
                    <div className={styles.overallTitle}>Overall Rating:</div>
                </div>
                <div className={styles.submit} onClick={(e) => handleSubmit()}>Submit</div>
            </ModalBody>
        </Modal>
    );
};
